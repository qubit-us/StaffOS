...
const TABS = [
  ...
  { id: 2, label: 'Agreement',        icon: Scale      },
];
...
export default function EditVendorModal({ vendorId, onClose }) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get(`/api/vendors/${vendorId}`).then(r => r.data),
  });
  ...
  useEffect(() => {
    if (data && !form) {
      ...
      setForm({
        ...
        placement_types:    terms.placement_types || [],
        status:             data.relationship_status || 'active',
        contract_type:      terms.contract_type || 'both',
        margin_cap:         terms.margin_cap ?? '',
        contract_start:     terms.contract_start || '',
        contract_end:       terms.contract_end   || '',
        preferred_visas:    terms.preferred_visas || [],
        notes:              terms.notes || '',
        account_manager_id: terms.account_manager_id || '',
      });
    }
  }, [data]);
  ...
  const handleSubmit = () => {
    ...
    mutate({
      ...
      terms: {
        ...
        margin_cap:         form.margin_cap ? parseFloat(form.margin_cap) : null,
        contract_start:     form.contract_start || null,
        contract_end:       form.contract_end   || null,
        specializations:    form.specializations,
        placement_types:    form.placement_types,
        preferred_visas:    form.preferred_visas,
                  ...
                  <FieldRow>
                    ...
                    <Field label="Website">
                      <input className="input" value={form.website}
                        onChange={e => set('website', e.target.value)} placeholder="https://vendor.com" />
                    </Field>
                  </FieldRow>
                  <FieldRow>
